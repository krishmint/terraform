### AWS KEY PAIR MANAGEMENT -- 2 ways 
# 1 - (secure)You manuaaly Create key pair(both) locally and refer the public from local file
# 2 - Terraform create key pair and store private key in Local( or S3 bucket)
#      (not secure as statefile have private key) but fullu AUTOMATED 
     

######################################################
# 1 - manualy Create key pair locally and refer the publickey from local file

# ssh-keygen -t rsa -b 4096 -f ~/dev-key (You get dev-key & dev-key.pub local file ) 
# Create AWS key pair
# resource "aws_key_pair" "main" {
#   key_name   = var.key_name
#   public_key = file("~/dev-key.pub")  ## refering public key from local file
...
######################################################
# 2 -Terraform create key pair and store private key in local
